...
nts::GateOr::GateOr(Pin *output, Pin *input1, Pin *input2) : AGate(output), input1(input1), input2(input2)
{
    if (this->input1)
...
bool nts::GateOr::preventLoop(AGate *gate, int pathLength) const
{
    AComponent *linked;
    Pin *input_link;
...
nts::Tristate nts::GateOr::compute()
{
    nts::Tristate input1;
    nts::Tristate input2;
    IComponent *linked1;
