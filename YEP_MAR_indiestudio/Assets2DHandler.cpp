...
Assets2DHandler::Assets2DHandler(std::map<Graphical::typeTexture, irr::video::ITexture*> textures, irr::video::IVideoDriver *driver)
{
...
unsigned Assets2DHandler::create2DAsset(Graphical::typeTexture typeTexture, float posX, float posY)
{
    Asset2D	*obj = new Asset2D;

    // std::cout << "ALED ALED "  << (Graphical::typeTexture)typeTexture << std::endl;
    obj->posX = posX;
    obj->posY = posY;
...
bool Assets2DHandler::modify2DAsset(Graphical::typeTexture typeTexture, unsigned id, float posX, float posY)
{
    std::list<Asset2D *>::iterator	it;
...
bool Assets2DHandler::displayAll2DAsset()
{
    ...
    for (it = this->Asset2DList.begin(); it != this->Asset2DList.end(); it++) {
        this->driver->draw2DImage((*it)->img, irr::core::position2d<irr::s32>((*it)->posX, (*it)->posY),
                                  irr::core::rect<irr::s32>(0, 0, (*it)->img->getSize().Width, (*it)->img->getSize().Height), 0,
