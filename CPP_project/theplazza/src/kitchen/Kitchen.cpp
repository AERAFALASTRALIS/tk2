...
std::string Kitchen::KitchenKeyGen()
{
    ...
    k.push_back("PythonLava");
    k.push_back("NinjaFrench");
    k.push_back("EpicBear");
    k.push_back("BloodPepito");
    k.push_back("DogColossus");
...
{
    while (_alive) {
        if ((_born + std::chrono::seconds(DURATION) <= Plazza::time.now() && (_cooksPerKitchen == (int)this->freeCooks()))) {
...
{
    std::cout << "Kitchen[" << _name << "]: is closing !" << std::endl;
    Log log("Kitchen[" + _name + "]: closed");
